...
// Signal / pattern tables live in ./accountFallback/signals.ts so this module stays
// inside its frozen size in config/quality/file-size-baseline.json. Pure data — no
// behaviour change. They are re-exported here because combo.ts, mimocode.ts and the
// other callers already import them from this path (single source of truth).
import {
  ...
  MODEL_ACCESS_DENIED_CODES,
  MODEL_ACCESS_DENIED_TYPES,
  MODEL_ACCESS_AMBIGUOUS_TYPES,
...
} from "./accountFallback/signals.ts";
export {
  ACCOUNT_DEACTIVATED_SIGNALS,
  CREDITS_EXHAUSTED_SIGNALS,
  OAUTH_INVALID_TOKEN_SIGNALS,
